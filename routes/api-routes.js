...
module.exports = function(app) {
    ...
    app.post("/search", (req, res) => {
        ...
        let drugTitle = req.body.name.replace(/\s/g, "+");
        axios.get(
            `https://api.fda.gov/drug/label.json?search=brand_name:${drugTitle}&limit=1`
        ).then(
            (response) => {
    ...
    app.get("*", (req, res) => {
    res.sendFile(path.join(__dirname, "../client/build.bk/static/index.html"));
    });
}